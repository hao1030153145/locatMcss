...
public class DriverManager2 extends DriverManagerImpl implements LinuxCommandImpl {
    ...
    private DriverManager2() {
        ...
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                System.out.println("进入线程循环获得设备信息成功");
                while (true) {
                    try {
                    ...
                    String getDataUrl = "/devicesInfo/getDevicesInfo.json";
                    Map<String, String> dataMap = new HashMap<String, String>();
                    dataMap.put("registServer", "mcss_2");
                    Object firstObject = CallRemoteServiceUtil.callRemoteService(this.getClass().getName(), baseServer + getDataUrl, "post", dataMap);
                    if (null != firstObject) {
                        String jsonString = firstObject.toString();
                        devicesInfoBos = JSON.parseArray(jsonString, DevicesInfoBo.class);
                    }
                    if (devicesInfoBos.size() != driverManagerList.size()) {
                        List<String> list = new ArrayList<>();
                        for (AppiumDriverManager appiumDriverManager : driverManagerList) {
                            list.add(appiumDriverManager.getDeviceInfo().getDevicesName());
                        }
                        int size = driverManagerList.size();
                        for (DevicesInfoBo devicesInfoBo : devicesInfoBos) {
                            if (!list.contains(devicesInfoBo.getDevicesName())) {
                                AppiumDriverManager appiumDriverManager = new AppiumDriverManager();
                                ...
                                appiumDriverManager.setStatus("0");
                                driverManagerList.add(appiumDriverManager);
                                System.out.println("发现新增添设备，现在的设备集合为=====" + JSON.toJSONString(driverManagerList));
                            }
                        }
                    }
                }
...
//            executeShell("/home/weiqi/tim_downlaod/android_sdk/android-sdk-linux_x86/platform-tools/adb connect " + deviceName, true);
    ...
    public AppiumDriverManager changeDevicesStatus(String status, String dataTypeId) {
        for (AppiumDriverManager appiumDriverManager : driverManagerList) {
            if (appiumDriverManager.getStatus().equals(status) && ("").equals(appiumDriverManager.getDatasourceTypeId())) {
